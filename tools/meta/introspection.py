    ...
    async def _list_all_tools(self, category: str = "", **kwargs) -> ToolResult:
        ...
        try:
            ...
            return ToolResult(
                ...
                message=f"🔧 {len(tools_info)} tool categories with {total_actions} total actions"
            )
        ...
        try:
            ...
            if action:
                ...
                return ToolResult(
                    ...
                    message=f"📋 {tool_name}.{action} - see parameters in data"
                )
            
            return ToolResult(
                status=ToolStatus.SUCCESS,
                data=tool_data,
                message=f"📋 {tool_name}: {len(tool_data.get('actions', []))} actions"
            )
    ...
    async def _search_capabilities(self, query: str, **kwargs) -> ToolResult:
        ...
        try:
            ...
            return ToolResult(
                ...
                message=f"🔍 Found {len(matches)} built-in matches, {len(script_matches)} extension scripts for '{query}'"
            )
    ...
    async def _analyze_request(self, user_request: str, **kwargs) -> ToolResult:
        ...
        try:
            ...
            return ToolResult(
                ...
                message=f"{'✅ Can handle' if can_handle else '⚠️ May need extension'}: matched {len(matched_keywords)} keywords, {len(potential_tools)} tools"
            )
    ...
    async def _get_capability_summary(self, **kwargs) -> ToolResult:
        ...
        try:
            ...
            return ToolResult(
                status=ToolStatus.SUCCESS,
                data=summary,
                message=f"🧠 {summary['total_tools']} tools, {summary['total_actions']} actions, {summary['extension_scripts']} extensions"
            )
    ...
    async def _generate_script(self, name: str, description: str, language: str,
        ...
        try:
            ...
            return ToolResult(
                ...
                message=f"✨ Created extension script: {name} ({language})"
            )
...
'''
    ...
    async def _save_script(self, script_id: str, code: str = "", **kwargs) -> ToolResult:
        ...
        try:
            ...
            return ToolResult(
                status=ToolStatus.SUCCESS,
                message=f"💾 Updated script: {script.name}"
            )
    ...
    async def _list_scripts(self, category: str = "", verified_only: bool = False, **kwargs) -> ToolResult:
        ...
        try:
            ...
            return ToolResult(
                status=ToolStatus.SUCCESS,
                data=scripts_list,
                message=f"📜 {len(scripts_list)} extension scripts"
            )
    ...
    async def _delete_script(self, script_id: str, **kwargs) -> ToolResult:
        ...
        try:
            ...
            return ToolResult(
                status=ToolStatus.SUCCESS,
                message=f"🗑️ Deleted script: {script.name}"
            )
    ...
    async def _test_script(self, script_id: str, **kwargs) -> ToolResult:
        ...
        try:
            ...
            if script.language == "python":
                ...
                return ToolResult(
                    status=ToolStatus.SUCCESS,
                    message=f"⚠️ No syntax checker for {script.language}, file exists"
                )
            ...
            if result.returncode == 0:
                return ToolResult(
                    status=ToolStatus.SUCCESS,
                    message=f"✅ Script {script.name} syntax is valid"
                )
            else:
                return ToolResult(
                    status=ToolStatus.ERROR,
    ...
    async def _find_similar_scripts(self, description: str, **kwargs) -> ToolResult:
        ...
        try:
            ...
            return ToolResult(
                status=ToolStatus.SUCCESS,
                data=matches[:5],
                message=f"🔍 Found {len(matches)} similar scripts"
            )
        ...
        try:
            ...
            if analysis.data.get("can_handle"):
                return ToolResult(
                    ...
                    message="✅ Existing tools can handle this request"
                )
            ...
            return ToolResult(
                ...
                message=f"💡 Suggest creating a {suggested_language} extension script"
            )
